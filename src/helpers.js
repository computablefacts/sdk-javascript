 ...
 */
helpers.goodFastHash = function (str, seed) {

  const newSeed = seed !== undefined ? seed : 0;
...
/**
 * Escapes special characters with special meaning in RegExp.
 *
 * @param {string} str the string to escape.
 * @return {string} the escaped string.
 * @memberOf module:helpers
 */
helpers.escapeSpecialCharacters = function (str) {
  return str.replace(/[.*+?^${}()|[\]\\]/g, '\\$&' /* the whole matched string */);
}

/**
 * Converts a string to a list of RegExp literals.
 *
 ...
 * @memberOf module:helpers
 */
helpers.toRegExp = function (str, flags) {
  ...
  const newFlags = flags !== undefined ? flags : 'im';
  const escapedString = helpers.escapeSpecialCharacters(str)
  .split(/[\s\u00a0]+/).join('(\\s|\u00a0)*');
 ...
 *
 * @param {string|number} str the string to clean.
 ...
 */
helpers.removeDiacritics = function (str, preserveStringLength) {

  if (!str) {
    return '';
  }

  str = '' + str; // Converts integers to string
 ...
 * @final
 */
helpers.Pattern = function (pattern, color) {
 ...
 * @final
 */
helpers.Highlight = function (match, raw, highlighted, page) {
  this.match = match;
  this.raw = raw;
 ...
 * @param {Array<helpers.Highlight>} snippets the snippet associated with each highlighted text fragment.
 * @memberOf module:helpers
 * @constructor
 * @struct
 * @final
 */
helpers.HighlightedText = function (text, snippets) {
  this.text = text;
 ...
 * @param {Array<helpers.Pattern>} patterns a set of patterns to match and highlight.
 * @return {helpers.HighlightedText} the highlighted text.
 * @memberOf module:helpers
 */
helpers.highlight = function (text, patterns) {

  if (!text || !patterns || patterns.length <= 0) {
    return new helpers.HighlightedText(text, []);
  }

  let highlightedText = text;
  text = helpers.removeDiacritics(text, true);
  ...
  const snippets = patterns.flatMap(pattern => {
    ...
    const newPattern = helpers.escapeSpecialCharacters(
        helpers.removeDiacritics(pattern.pattern, true));
    ...
    return new helpers.Highlight(infix, rawSnippet, highlightedSnippet,
        pages.length);
  });
  return new helpers.HighlightedText(highlightedText, snippets);
}

/**
 * Returns true iif a string is null or blank, false otherwise.
 *
